...
async def publish_discovered_article(option: DiscoveryOption):
    """
...
async def list_links():
    """Return all published content cards, newest first."""
    return get_all_links()


@app.get("/api/links/{link_id}", response_model=ContentCard)
async def get_single_link(link_id: str):
    """Return a single content card by ID (used by the share page)."""
    card = get_link(link_id)
    if not card:
