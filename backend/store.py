...
def get_all_links() -> list[dict]:
    """Return all stored content cards from Supabase, newest first."""
    try:
        response = _get_supabase().table("links")\
            .select("*")\
            .order("created_at", desc=True)\
            .execute()
        return response.data
    except Exception as e:
        logging.error(f"Error fetching links from Supabase: {e}")
        return []

def get_link(link_id: str) -> dict | None:
    """Return a single content card by its ID from Supabase, or None if not found."""
    try:
        response = _get_supabase().table("links")\
            .select("*")\
            .eq("id", link_id)\
        ...
        logging.error(f"Error fetching link {link_id} from Supabase: {e}")
    return None

def save_link(card: dict) -> dict:
    """
    Save a new content card to Supabase.
    ...
    try:
        ...
        logging.error(f"Error saving link to Supabase: {e}")
        # Fallback: re-raise or handle as needed
        raise e

def delete_link(link_id: str) -> bool:
    """Delete a content card by ID from Supabase. Returns True if successful."""
    try:
        response = _get_supabase().table("links")\
            .delete()\
            .eq("id", link_id)\
        ...
        logging.error(f"Error deleting link {link_id} from Supabase: {e}")
        return False
